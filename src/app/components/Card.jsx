    ...
    <div className="flex flex-col-reverse md:flex-row items-center justify-center w-full h-screen overflow-hidden px-4">
      ...
      <div className="relative flex flex-col border border-[#db9125] rounded-lg shadow-lg custom-gradient" style={{ width: "100%", maxWidth: "550px", height: "320px" }}>
        ...
        <div className="flex flex-row items-center mt-10 p-4">
          <div className="flex-shrink-0">
            <Image src={cardData[currentIndex].imageUrl} alt={cardData[currentIndex].name} width={120} height={120} className="object-cover border rounded-lg border-gray-500" />
          </div>
          <div className="flex flex-col ml-4">
            <h3 className="text-xl md:text-2xl text-[#db9125]">{cardData[currentIndex].name}</h3>
            <p className="text-normal md:text-xl text-white my-2">{cardData[currentIndex].role}</p>
            <p className="text-gray-200 text-xs md:text-sm">{cardData[currentIndex].details}</p>
          </div>
        </div>
        <div className="p4">
      ...
      <div className="flex flex-col items-start md:ml-10 mt-6 md:mt-0 px-4 mx-4 mb-6">
        ...
        <div className="flex items-center">
          <button onClick={handlePrev} className="text-black font-bold text-xl px-2 py-1" style={{ visibility: currentIndex > 0 ? "visible" : "hidden" }}>
            {"←"}
          </button>

          <div className="flex flex-row overflow-x-auto no-scrollbar space-x-2" style={{ width: "100%", maxWidth: "360px" }}>
            {currentCards.map((item, index) => {
              const cardIndex = start + index;
              const isActive = cardIndex === currentIndex;
              return (
                <div key={cardIndex} className={`w-20 h-20 border rounded-lg ${isActive ? "border-[#db9125]" : "border-gray-400"} ${isActive ? "" : "filter blur-sm"}`} style={{ position: "relative" }} onClick={() => handleImageClick(index)}>
                  <Image src={item.imageUrl} alt={item.name} layout="fill" objectFit="cover" className="rounded-lg cursor-pointer" />
                </div>
              );
            })}
          </div>

          <button onClick={handleNext} className="text-black font-bold text-xl px-2 py-1" style={{ visibility: currentIndex < cardData.length - 1 ? "visible" : "hidden" }}>
            {"→"}
          </button>
        </div>
      </div>
